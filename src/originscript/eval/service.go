...
func (s *ServiceObj) CallMethod(line string, scope *Scope, method string, args ...Object) Object {
	switch method {
		...
		case "Show":
			return s.Print_Status(line, args...)
	}
	return NewError(line, NOMETHODERROR, method, s.Type())
}

func (s *ServiceObj) Print_Status(line string, args ...Object) Object {
	if len(args) != 1 {
		return NewError(line, ARGUMENTERROR, "1", len(args))
	}
	 
	fmt.Printf("Request to %s recieved",args[0])
...
func (s *ServiceObj) Run(line string, args ...Object) Object {
	...
	//if args[0].(*Boolean).Bool {
	//	s.Router.Use(LoggingMiddlewareAdvanced)
	//}

	//if args[0].(*Boolean).Bool {
	//	s.Router.Use(LoggingMiddlewareMoreAdvanced)
	//}
...
// Below code will cause panic(mainly request body handling)
func LoggingMiddlewareAdvanced(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		//fmt.Printf("* %-7s %s\n", req.Method, req.URL.Path)
		fmt.Printf("[NEW] ADVANCED Request url: %s\n", req.URL)
...
// Below code will cause panic(mainly request body handling)
func LoggingMiddlewareMoreAdvanced(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		//fmt.Printf("* %-7s %s\n", req.Method, req.URL.Path)
		fmt.Printf("[NEW] MORE ADVANCED Request url: %s\n", req.URL)
